...
- **Stale `log_path` after migration** - A migrated `config.json` kept its old `log_path` value (`/data/govee-ble/logs/govee_ble.log`), causing the service to recreate that directory and log there instead of the new `setupOptions` location. `setup install auto` now unconditionally self-heals `log_path` to the correct location on every run.
- **`rc.local` cleanup didn't match a real deployed install** - The removal pattern was built from the `install.sh` version in git history, but an actually-deployed instance had a different-format boot hook that the old pattern silently failed to match (while still logging success). Broadened to match any line containing `govee-ble`, and the log message now reflects the real line count instead of firing unconditionally.
- **Orphaned `supervise` process after removing the old service** - `svc -d` only stopped the old service; the underlying `supervise` process kept running indefinitely. Changed to `svc -dx`, which tells `supervise` to exit once the service is down.
